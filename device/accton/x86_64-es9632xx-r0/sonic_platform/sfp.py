...
class Sfp(SfpBase):
    ...
    def _read_eeprom_specific_bytes(self, offset, num_bytes):
        eeprom_raw = []
        try:
            eeprom = open(self._eeprom_path, mode="rb", buffering=0)
        except IOError:
            return None
        ...
        if self.sfp_type == QSFP_TYPE:
            ...
            if qsfp_dom_capability_raw is not None:
                ...
                if dom_capability['data']['Flat_MEM']['value'] == 'Off':
                    self.dom_supported = True
                    # Currently set to False becasue kernel driver does not support access to this page
    ...
    def get_transceiver_info(self):
        ...
        if self.sfp_type == OSFP_TYPE:
            ...
            sfpi_obj = sff8436InterfaceId()
            if sfpi_obj is None:
                logger.log_error("Error: sfp_object sff8436InterfaceId open failed")
                return transceiver_info_dict
            ...
            sfpi_obj = qsfp_dd_InterfaceId()
            if sfpi_obj is None:
                logger.log_error("Error: sfp_object qsfp_dd_InterfaceId open failed")
                return transceiver_info_dict
            ...
            sfpi_obj = sff8472InterfaceId()
            if sfpi_obj is None:
                logger.log_error("Error: sfp_object sff8472InterfaceId open failed")
                return transceiver_info_dict
            ...
            if self.sfp_type == QSFP_TYPE:
                ...
                if sfp_ext_specification_compliance_raw is not None:
                    ...
                    if sfp_ext_specification_compliance_data['data']['Extended Specification compliance']['value'] != "Unspecified":
                        compliance_code_dict['Extended Specification compliance'] = \
                            sfp_ext_specification_compliance_data['data']['Extended Specification compliance']['value']
    ...
    def get_transceiver_bulk_status(self):
        ...
        ========================================================================
        """
        transceiver_dom_info_dict = dict.fromkeys(dom_info_dict_keys, NULL_VAL)
        ...
        if self._port_num > self.PORT_END:
            ...
            try:
                ...
                if not self.get_presence():
                    return False
                # Write to eeprom
                eeprom = open(self._eeprom_path, "rb")
    ...
    def set_power_override(self, power_override, power_set):
        ...
        if self._port_num > self.PORT_END:
            ...
            try:
                ...
                with open(self._eeprom_path, "r+b") as fd:
